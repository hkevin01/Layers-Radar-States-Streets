			...
			try {
				...
				if (okSize && view && view.calculateExtent) {
					...
					if (okExtent && window.ol?.proj?.transformExtent) {
						...
						okExtent = Array.isArray(ex4326) && ex4326.every(Number.isFinite);
						if (okExtent) { window.testHelper.info.extent4326 = ex4326; }
					}
				}
				if (okSize && okExtent) { window.testHelper.mapReady = true; }
